...
2. After the first implementation receives Verifier PASS, save a reusable Skill named `normalize-portable-path`.
   ...
   - Its free-form Markdown content must make clear that it applies to converting mixed Windows/POSIX separators into stable forward-slash paths.
   - Frontmatter must contain `name: normalize-portable-path`; all other frontmatter fields and body sections are optional.
   - The content must include implementation steps and an independent verification step. No `Instructions` or `Examples` section is required.
